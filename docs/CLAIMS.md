...
| Vizier evaluation mode cannot return `ALLOW`. | local tests | REST, A2A, and MCP tests assert `REVIEW` without trusted authority; A2A tests also cover anonymous evaluation while `VIZIER_API_KEY` is configured and rejection of a wrong credential. | supported |
