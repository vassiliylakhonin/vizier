...
import { checkOfacAddress } from "./ofac";
import { z } from "zod";
...
export async function handleReviews(request: Request, options: TransportOptions): Promise<Response> {
  ...
  if (url.pathname === "/v1/reviews") {
    ...
    if (action) await checkOfacAddress(options.circuitBreakerKv, action.recipient);
  ...
  if (match[2] === "decision") {
    ...
    const action = financialAction(parse(reviewSubmissionSchema, JSON.parse(row.payload_json) as unknown));
    if (action && input.decision === "APPROVED") await checkOfacAddress(options.circuitBreakerKv, action.recipient);
